...
long distance_in_cm_R;      ////////////////////////////trigpin and echopin for both LR not sure.
#define trigPin_L A2
#define echoPin_L A3
#define trigPin_R A2
#define echoPin_R A3
...
distance_in_cm_L = (duration/2.0) / 29.1;
}
...
distance_in_cm_R = (duration/2.0) / 29.1;
}

//xy position from the feedback is
...
//havent consider case for too close, fault data of 1200. this can be fixed by checking reading from voltmeter.

void movement(long distance_L,long distance_R){
  if ((distance_L > 28) or (distance_R > 28)){
    ADVANCE();
  }
  else{
    STOP();
...
void UART_Control()
{
  ...
  if (SERIAL.available())
  {
    ...
    //Search for the next comma just after the fiwuwirst
...
void loop()
{
  ...
  if (millis() > (time + 15)) {
    ...
    measure_distance_L();
    measure_distance_R();
    movement(distance_in_cm_L, distance_in_cm_R);
